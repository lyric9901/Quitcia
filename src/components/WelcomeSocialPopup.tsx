...
export default function WelcomeSocialPopup() {
  ...
  useEffect(() => {
    // Check if the user has already seen the screen
  ...
  // Saves to local storage, but leaves the screen visible so they can click the other link
  const markAsSeen = () => {
    localStorage.setItem("hasSeenSocialPopup", "true");
  };

  // Saves to local storage AND hides the full screen
  const handleClose = () => {
    markAsSeen();
    setIsVisible(false);
  };
      ...
      <div className="flex w-full max-w-sm flex-1 flex-col items-center justify-center mx-auto">
        ...
        <div className="flex w-full flex-col gap-4">
          <a
            ...
            onClick={markAsSeen}
            className="flex w-full items-center justify-center gap-3 rounded-2xl bg-[#FF4500] px-4 py-4 text-[16px] font-semibold text-white shadow-lg shadow-[#FF4500]/20 transition-all hover:bg-[#FF4500]/90 active:scale-[0.98]"
          >
          ...
          <a
            ...
            rel="noopener noreferrer"
            onClick={markAsSeen}
            className="flex w-full items-center justify-center gap-3 rounded-2xl bg-[#2AABEE] px-4 py-4 text-[16px] font-semibold text-white shadow-lg shadow-[#2AABEE]/20 transition-all hover:bg-[#2AABEE]/90 active:scale-[0.98]"
          >
            <svg width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
